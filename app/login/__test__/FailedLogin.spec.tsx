...
jest.mock('next-auth/react', () => {
    ...
    return {
        ...
        signIn: jest.fn().mockImplementation(() =>
            Promise.resolve({ error: 'Internal error', status: 403, ok: false, url: '' })
        )
    }
});

describe("Failed login for server error test", () => {
    beforeEach(() => {
        jest.clearAllMocks()
    })

    it("Should show failed to login message if sign in failed", async () => {
